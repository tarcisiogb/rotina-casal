...
function renderGraficoCorpoCard() {
  ...
  ['tarcisio', 'aline'].forEach(function (quem) {
    ...
    for (var i = 0; i < arr.length; i++) {
      var v = null;
      if (metrica === 'imc') v = calcIMC(arr[i].peso, arr[i].altura);
      else if (metrica === 'rcq') v = calcRCQ(arr[i].cintura, arr[i].quadril);
      else if (metrica === 'rcest') v = calcRCEst(arr[i].cintura, arr[i].altura);
...
function renderListaCorpo() {
  ...
  for (var k = 0; k < todos.length; k++) {
    var reg = todos[k];
    var quemLabel = reg._quem === 'tarcisio' ? '👨 Tarcísio' : '👩 Aline';
    var imc = calcIMC(reg.peso, reg.altura);
    var rcq = calcRCQ(reg.cintura, reg.quadril);
    var rcest = calcRCEst(reg.cintura, reg.altura);
    html += '<div class="list-item"><div>';
    ...
    if (fotosReg.length > 0) {
      ...
      for (var fi = 0; fi < fotosReg.length; fi++) html += '<img src="' + fotosReg[fi] + '" style="width:80px;height:80px;object-fit:cover;border-radius:8px;">';
      html += '</div>';
    }
    if (reg.obs) html += '<div class="li-sub">' + esc(reg.obs) + '</div>';
...
async function salvarCorpo() {
  ...
  var reg = {
    ...
    peso: peso,
    altura: Number(document.getElementById('corpoAltura').value) || null,
