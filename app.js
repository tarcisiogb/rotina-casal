...
var TIPOS_SAUDE = [
  ...
  { v: 'concentracao', l: '🧠 Concentração' }, { v: 'outro', l: '➕ Outro' }
];
...
function renderPeriodosCard() {
  ...
  html += '</div></div>';
  html += '<div class="field"><label>Tipo</label><div class="pill-row">';
  html += '<button type="button" class="tipo-pill' + pillOn('periodoTipo', 'remedio') + '" onclick="selectSingle(\'periodoTipo\',\'remedio\',this);renderSaude();">💊 Remédio</button>';
  html += '<button type="button" class="tipo-pill' + pillOn('periodoTipo', 'atividade') + '" onclick="selectSingle(\'periodoTipo\',\'atividade\',this);renderSaude();">🏃 Atividade</button>';
  html += '</div></div>';
  ...
  html += '<div class="field"><label>Nome</label><input class="input" id="periodoNome" placeholder="Ex: Sertralina 50mg" value="' + eb('periodo', 'nome') + '"></div>';
  html += '<div class="field"><label>Início</label>' + campoModalHtml('periodoDataInicio', 'date', valorPendente('periodoDataInicio', eb('periodo', 'dataInicio', hojeISO())), 'Início do uso', 'renderSaude') + '</div>';
  html += '<div class="field"><label>Fim (deixe vazio se ainda está em uso)</label>' + campoModalHtml('periodoDataFim', 'date', valorPendente('periodoDataFim', eb('periodo', 'dataFim')), 'Fim do uso', 'renderSaude') + '</div>';
  html += '<div class="field"><label>Observação (opcional)</label><textarea class="input" id="periodoObs">' + eb('periodo', 'obs') + '</textarea></div>';
...
function renderInsightAlcoolCard() {
  var linhas = [];
  var referencia = { tarcisio: 14, aline: 7 }; // NIAAA/CDC: até 14 doses/semana homens, até 7 mulheres
  ['tarcisio', 'aline'].forEach(function (quem) {
    var nomeLabel = quem === 'tarcisio' ? 'Tarcísio' : 'Aline';
    ...
    var meta = referencia[quem];
    var frase = nomeLabel + ': ' + doses7 + ' dose(s) nos últimos 7 dias (referência de consumo moderado: até ' + meta + '/semana). ';
    frase += doses7 <= meta ? 'Dentro da faixa considerada moderada.' : 'Acima da faixa considerada moderada.';
    if (mediaAnterior !== null) {
  ...
  linhas.push('<span style="font-style:italic;">Referência de consumo moderado (NIAAA/CDC) — a OMS considera que nenhuma quantidade de álcool é totalmente livre de risco.</span>');
  return '<div class="card ai-box"><div class="card-title">💡 Insight</div>' + linhas.join('<br><br>') + '</div>';
}
...
function renderInsightIntimidadeCard() {
  var arr = window.ST.intimidade.encontros;
  var freq7 = 0, freqAnteriores = 0;
  for (var i = 0; i < arr.length; i++) {
    var d = diffDias(arr[i].data, hojeISO());
    if (d >= 0 && d <= 6) freq7++;
    else if (d >= 7 && d <= 13) freqAnteriores++;
  }
  var frase = freq7 + ' encontro(s) nos últimos 7 dias';
  if (freqAnteriores > 0 || freq7 > 0) {
    if (freq7 > freqAnteriores) frase += ', mais que na semana anterior (' + freqAnteriores + ').';
    else if (freq7 < freqAnteriores) frase += ', menos que na semana anterior (' + freqAnteriores + ').';
    else frase += ', igual à semana anterior.';
  } else {
    frase += '.';
  }
...
function renderCruzamentoLazerIntimidadeCard() {
  var html = '<div class="card"><div class="card-title">🔗 Lazer × Intimidade</div>';
  html += '<div class="card-sub">Frequência das duas coisas, semana a semana, para ver se uma acompanha a outra.</div>';
  html += '<table class="tbl"><tr><th>Semana</th><th>Lazer</th><th>Intimidade</th></tr>';
  for (var s = 7; s >= 0; s--) {
    var inicioJanela = s * 7, fimJanela = s * 7 + 6;
    var lazerCount = 0, intimCount = 0;
    ...
    for (var j = 0; j < window.ST.intimidade.encontros.length; j++) {
      ...
      if (d2 >= inicioJanela && d2 <= fimJanela) intimCount++;
    }
    var rotulo = s === 0 ? 'Esta semana' : 'Há ' + s + ' semana(s)';
    html += '<tr><td>' + rotulo + '</td><td>' + lazerCount + '</td><td>' + intimCount + '</td></tr>';
  }
  html += '</table></div>';
  return html;
}
