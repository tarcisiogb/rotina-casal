...
// ===================== CORPO / MEDIDAS =====================
window._corpoFotoTemp = { blob: null, previewUrl: null };
...
function renderCorpo() {
  ...
  html += '<div class="field"><label>Peso (kg)</label><input class="input" type="number" step="0.1" id="corpoPeso" value="' + eb('corpo', 'peso') + '"></div>';
  html += '<div class="field"><label>Altura (cm) — informe uma vez, será lembrada</label><input class="input" type="number" id="corpoAltura" value="' + eb('corpo', 'altura', ultimaAlturaCorpo(window._sel.corpoQuem)) + '"></div>';
  html += '<div class="field"><label>Cintura (cm, opcional)</label><input class="input" type="number" step="0.1" id="corpoCintura" value="' + eb('corpo', 'cintura') + '"></div>';
  html += '<div class="field"><label>Quadril (cm, opcional)</label><input class="input" type="number" step="0.1" id="corpoQuadril" value="' + eb('corpo', 'quadril') + '"></div>';
  html += '<div class="field"><label>Coxa (cm, opcional)</label><input class="input" type="number" step="0.1" id="corpoCoxa" value="' + eb('corpo', 'coxa') + '"></div>';
  html += '<div class="field"><label>Braço (cm, opcional)</label><input class="input" type="number" step="0.1" id="corpoBraco" value="' + eb('corpo', 'braco') + '"></div>';
  html += '<div class="field"><label>Foto (opcional)</label>';
  html += '<input type="file" accept="image/*" id="corpoFotoInput" class="input" onchange="selecionarFotoCorpo(this)">';
  if (window._corpoFotoTemp.previewUrl) html += '<img src="' + window._corpoFotoTemp.previewUrl + '" style="max-width:120px;border-radius:10px;margin-top:8px;display:block;">';
  else if (editando && eb('corpo', 'fotoUrl')) html += '<img src="' + eb('corpo', 'fotoUrl') + '" style="max-width:120px;border-radius:10px;margin-top:8px;display:block;">';
  html += '</div>';
  html += '<div class="field"><label>Observação (opcional)</label><textarea class="input" id="corpoObs">' + eb('corpo', 'obs') + '</textarea></div>';
...
function selecionarFotoCorpo(input) {
  ...
  comprimirImagem(input.files[0], function (blob) {
    window._corpoFotoTemp.blob = blob;
    window._corpoFotoTemp.previewUrl = URL.createObjectURL(blob);
    renderCorpo();
  });
}
...
function renderListaCorpo() {
  ...
  for (var k = 0; k < todos.length; k++) {
    ...
    if (reg.fotoUrl) html += '<img src="' + reg.fotoUrl + '" style="max-width:90px;border-radius:8px;margin-top:6px;">';
    if (reg.obs) html += '<div class="li-sub">' + esc(reg.obs) + '</div>';
...
function editarCorpo(quem, id) {
  ...
  window._editBuffer.corpo = { data: reg.data, peso: reg.peso, altura: reg.altura, cintura: reg.cintura, quadril: reg.quadril, coxa: reg.coxa, braco: reg.braco, obs: reg.obs, fotoUrl: reg.fotoUrl };
  window._corpoFotoTemp = { blob: null, previewUrl: null };
...
async function salvarCorpo() {
  ...
  var reg = {
    ...
    fotoUrl: (editando ? eb('corpo', 'fotoUrl') : null) || null,
    obs: document.getElementById('corpoObs').value || '',
    registradoPor: window._userName || null
  };

  if (window._corpoFotoTemp.blob) {
    try {
      var caminho = 'corpo/' + quem + '/' + reg.id + '.jpg';
      reg.fotoUrl = await window._uploadFoto(window._corpoFotoTemp.blob, caminho);
    } catch (e) {
      alert('Não foi possível enviar a foto, mas o resto do registro será salvo. Tente reenviar a foto editando este registro.');
    }
  }
