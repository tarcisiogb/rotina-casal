...
var TIPOS_TREINO = [
  ...
  { v: 'futebol', l: '⚽ Futebol' }, { v: 'outro', l: '➕ Outro' }
];
...
var TIPOS_JUNTOS = [
  ...
  { v: 'serie_filme', l: '📺 Série/filme' }, { v: 'passeio', l: '🚶 Passeio juntos' }, { v: 'outro', l: '➕ Outro' }
];
...
function renderListaJuntos() {
  ...
  for (var i = 0; i < arr.length; i++) {
    var reg = arr[i];
    html += '<div class="list-item"><div>';
    html += '<div class="li-main">' + tipoJuntosLabel(reg.tipo) + ' — ' + (reg.quem === 'familia' ? 'Família' : 'Casal') + '</div>';
...
function renderIntimidade() {
  ...
  html += '<button type="button" class="tipo-pill" onclick="selectSingle(\'intimLocal\',\'quarto\',this)">🛏️ Quarto</button>';
  html += '<button type="button" class="tipo-pill" onclick="selectSingle(\'intimLocal\',\'sala\',this)">🛋️ Sala</button>';
  html += '<button type="button" class="tipo-pill" onclick="selectSingle(\'intimLocal\',\'outro\',this)">🛁 Outro</button>';
  html += '</div></div>';
...
function renderListaIntimidade() {
  ...
  var html = '';
  if (arr.length === 0) html = '<div class="li-sub">Nenhum registro ainda.</div>';
  var localLabel = { quarto: '🛏️ Quarto', sala: '🛋️ Sala', outro: '🛁 Outro' };
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    html += '<div class="li-sub">' + (inicLabel[reg.iniciativa] || '') + ' · ' + (reg.consumado === 'sim' ? '✅' : '❌') + ' · ' + (localLabel[reg.local] || '') + (fase ? ' · ' + fase.emoji + ' ' + fase.fase : '') + '</div>';
...
function salvarIntimidade() {
  ...
  if (!iniciativa || !consumado || !local) { alert('Preencha iniciativa, consumado e local'); return; }
  var reg = {
    id: uid(), data: document.getElementById('intimData').value || hojeISO(),
    hora: document.getElementById('intimHora').value || agoraHora(),
    iniciativa: iniciativa, consumado: consumado, local: local,
...
var TIPOS_BEBIDA = [
  ...
  { v: 'destilado', l: '🥃 Destilado' }, { v: 'drink', l: '🍹 Drink' }, { v: 'outro', l: '🫧 Outro' }
];
