...
function renderGraficoCorpoCard() {
  ...
  var opcoes = [['peso', 'Peso'], ['imc', 'IMC'], ['cintura', 'Cintura'], ['quadril', 'Quadril'], ['braco', 'Braço']];
...
function renderListaCorpo() {
  ...
  for (var k = 0; k < todos.length; k++) {
    ...
    if (reg.cintura || reg.quadril || reg.braco) {
      html += '<div class="li-sub">' + (reg.cintura ? 'Cintura: ' + reg.cintura + 'cm ' : '') + (reg.quadril ? '· Quadril: ' + reg.quadril + 'cm ' : '') + (reg.braco ? '· Braço: ' + reg.braco + 'cm' : '') + '</div>';
    }
...
function editarCorpo(quem, id) {
  ...
  window._editBuffer.corpo = { data: reg.data, peso: reg.peso, altura: reg.altura, cintura: reg.cintura, quadril: reg.quadril, braco: reg.braco, obs: reg.obs, fotoUrl: reg.fotoUrl };
  window._corpoFotoTemp = { blob: null, previewUrl: null };
