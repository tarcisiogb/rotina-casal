...
function classificarRCEst(rcest) {
  ...
  return rcest < 0.5 ? 'Baixo risco' : 'Risco aumentado';
}
...
function renderGraficoCorpoCard() {
  ...
  var html = '<div class="card"><div class="card-title">📈 Evolução</div>';
...
function renderListaCorpo() {
  ...
  for (var k = 0; k < todos.length; k++) {
    ...
    html += '<div class="li-sub">Peso: ' + (reg.peso || '—') + 'kg' + (imc ? ' · IMC: ' + fmt1(imc) + ' (' + classificarIMC(imc) + ')' : '') + '</div>';
    ...
    if (rcest) html += '<div class="li-sub">RCEst: ' + rcest.toFixed(2) + ' (' + classificarRCEst(rcest) + ')</div>';
