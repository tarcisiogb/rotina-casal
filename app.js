...
var DATA_NASCIMENTO = { tarcisio: '1986-04-24', aline: '1986-02-16' };
function calcularIdade(quem) {
  var nasc = DATA_NASCIMENTO[quem];
  if (!nasc) return null;
  var hoje = new Date();
  var nascD = new Date(nasc + 'T00:00:00');
  var idade = hoje.getFullYear() - nascD.getFullYear();
  var m = hoje.getMonth() - nascD.getMonth();
  if (m < 0 || (m === 0 && hoje.getDate() < nascD.getDate())) idade--;
  return idade;
}
function classificarIMC(imc, quem) {
  if (imc === null) return '';
  var idade = calcularIdade(quem);
  var tetoNormal = (idade !== null && idade >= 65) ? 27 : 24.9;
  if (imc < 18.5) return 'Abaixo do peso';
  if (imc <= tetoNormal) return 'Peso normal';
  if (imc < 30) return 'Sobrepeso';
  return 'Obesidade';
}
function classificarRCQ(rcq, quem) {
  if (rcq === null) return '';
  var limite = quem === 'aline' ? 0.85 : 0.90; // referência OMS: mulheres 0.85 / homens 0.90
  return rcq <= limite ? 'Risco baixo' : 'Risco aumentado';
}
function calcRCEst(cintura, alturaCm) {
  if (!cintura || !alturaCm) return null;
  return cintura / alturaCm;
}
function classificarRCEst(rcest, quem) {
  if (rcest === null) return '';
  var idade = calcularIdade(quem);
  var limite = (idade !== null && idade >= 50) ? 0.6 : 0.5;
  return rcest < limite ? 'Baixo risco' : 'Risco aumentado';
}

// Gera um gráfico de linha simples em SVG (sem dependências externas)
function renderGraficoLinha(pontos, corLinha, formatador) {
  if (!pontos || pontos.length < 2) return '<div class="li-sub">Registre pelo menos 2 medições para ver o gráfico.</div>';
  var fmt = formatador || fmt1;
  var W = 320, H = 140, PAD = 24;
  var valores = [];
  for (var i = 0; i < pontos.length; i++) valores.push(pontos[i].y);
  var min = Math.min.apply(null, valores), max = Math.max.apply(null, valores);
  if (min === max) { min -= 1; max += 1; }
  var passoX = (W - PAD * 2) / (pontos.length - 1);
  var coords = [];
  for (var j = 0; j < pontos.length; j++) {
    var x = PAD + passoX * j;
    var y = H - PAD - ((pontos[j].y - min) / (max - min)) * (H - PAD * 2);
    coords.push(x + ',' + y);
  }
  var svg = '<svg viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;height:auto;background:var(--bg);border-radius:10px;">';
  svg += '<polyline points="' + coords.join(' ') + '" fill="none" stroke="' + corLinha + '" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>';
  for (var k = 0; k < coords.length; k++) {
    var xy = coords[k].split(',');
    svg += '<circle cx="' + xy[0] + '" cy="' + xy[1] + '" r="3.5" fill="' + corLinha + '"/>';
  }
  svg += '<text x="' + PAD + '" y="14" font-size="11" fill="var(--muted)">' + fmt(max) + '</text>';
  svg += '<text x="' + PAD + '" y="' + (H - 10) + '" font-size="11" fill="var(--muted)">' + fmt(min) + '</text>';
  svg += '</svg>';
  return svg;
}

// Redimensiona/comprime a imagem no navegador antes do upload (mantém o Storage gratuito por muito mais tempo)
...
function abrirModalCampo(campoId, tipo, valorAtual, titulo, callbackNome) {
  ...
  var btnAgora = document.getElementById('modalCampoAgoraBtn');
  if (tipo === 'time') btnAgora.textContent = 'Usar horário atual';
  else if (tipo === 'date') btnAgora.textContent = 'Usar hoje';
  else btnAgora.textContent = 'Usar agora';
  abrirModal('modal-campo');
}

function preencherAgoraModalCampo() {
  var input = document.getElementById('modalCampoInput');
  var tipo = input.type;
  var val;
  if (tipo === 'time') val = agoraHora();
  else if (tipo === 'date') val = hojeISO();
  else if (tipo === 'datetime-local') val = hojeISO() + 'T' + agoraHora();
  else return;
  input.value = val;
  sincronizarValorModalCampo(input);
  confirmarModalCampo();
}
window.preencherAgoraModalCampo = preencherAgoraModalCampo;
...
function cancelarEdicao(modulo) {
  ...
  window._sel = {};
  if (modulo === 'treino') window._treinoCamposTemp = {};
...
window.abrirFotoAmpliada = abrirFotoAmpliada;
...
function renderSono() {
  ...
  html += renderGraficoBarrasSono();
  html += '<div class="card"><div class="card-title">Histórico</div><div id="sonoLista"></div></div>';
  el.innerHTML = html;
  renderListaSono();
}

function renderInsightSonoCard() {
  var html = '<div class="card ai-box"><div class="card-title">💡 Insight</div>';
  var linhas = [];
  ['tarcisio', 'aline'].forEach(function (quem) {
    var nomeLabel = quem === 'tarcisio' ? 'Tarcísio' : 'Aline';
    var arr = window.ST[quem].sono;
    if (arr.length === 0) { linhas.push(nomeLabel + ': nenhum registro de sono ainda.'); return; }
    var ordenadoDesc = arr.slice().sort(function (a, b) { return a.data < b.data ? 1 : -1; });
    var ultimo = ordenadoDesc[0];
    var diasSemRegistro = diffDias(ultimo.data, hojeISO());

    var recentes = [], anteriores = [];
    for (var i = 0; i < arr.length; i++) {
      var d = diffDias(arr[i].data, hojeISO());
      if (d >= 0 && d <= 6) recentes.push(arr[i]);
      else if (d >= 7 && d <= 13) anteriores.push(arr[i]);
    }
    var minsRecentes = [], minsAnteriores = [];
    for (var j = 0; j < recentes.length; j++) { var mr = calcSonoReal(recentes[j].dormir, recentes[j].acordar, recentes[j].intervalo); if (mr !== null) minsRecentes.push(mr); }
    for (var k = 0; k < anteriores.length; k++) { var ma = calcSonoReal(anteriores[k].dormir, anteriores[k].acordar, anteriores[k].intervalo); if (ma !== null) minsAnteriores.push(ma); }
    var mediaRecente = media(minsRecentes), mediaAnterior = media(minsAnteriores);

    var frase = nomeLabel + ': ';
    if (mediaRecente !== null) {
      frase += 'média de ' + minParaHoras(mediaRecente) + ' de sono real por noite nos últimos 7 dias';
      if (mediaAnterior !== null) {
        var diffMin = mediaRecente - mediaAnterior;
        if (Math.abs(diffMin) < 10) frase += ', estável em relação à semana anterior.';
        else if (diffMin > 0) frase += ', melhorando em relação à semana anterior (+' + Math.round(diffMin) + 'min).';
        else frase += ', piorando em relação à semana anterior (' + Math.round(diffMin) + 'min).';
      } else {
        frase += '.';
      }
    } else {
      frase += 'sem registros de sono nos últimos 7 dias.';
    }
    if (diasSemRegistro >= 2) frase += ' ⚠️ Já se passaram ' + diasSemRegistro + ' dias sem nenhum registro novo.';
    linhas.push(frase);
  });
  html += linhas.join('<br><br>');
  html += '</div>';
  return html;
}
...
var TIPOS_TREINO = [
  ...
  { v: 'pilates', l: '🤸 Pilates' }, { v: 'calistenia', l: '🤸‍♂️ Calistenia' },
  { v: 'outro', l: '➕ Outro' }
];
...
function renderTreino() {
  ...
  for (var i = 0; i < TIPOS_TREINO.length; i++) {
    html += '<button type="button" class="tipo-pill' + pillOn('treinoTipo', TIPOS_TREINO[i].v) + '" onclick="capturarCamposTreino();selectSingle(\'treinoTipo\',\'' + TIPOS_TREINO[i].v + '\',this);renderTreino();">' + TIPOS_TREINO[i].l + '</button>';
  }
  html += '</div></div>';
  ...
  if (window._sel.treinoTipo === 'corrida') {
    html += '<div class="field"><label>Distância (km)</label><input class="input" type="number" step="0.01" id="treinoDistancia" value="' + valorCampoTreino('treinoDistancia', eb('treino', 'distancia')) + '"></div>';
    html += '<div class="field"><label>Frequência cardíaca média (bpm, opcional)</label><input class="input" type="number" id="treinoFcMedia" value="' + valorCampoTreino('treinoFcMedia', eb('treino', 'fcMedia')) + '"></div>';
    html += '<div class="field"><label>Calorias (opcional)</label><input class="input" type="number" id="treinoCalorias" value="' + valorCampoTreino('treinoCalorias', eb('treino', 'calorias')) + '"></div>';
  }
  ...
  html += renderGraficoBarrasTreino();
  html += renderPerformanceCorridaCard();
...
function calcularSequenciaTreino(arr) {
  var datasComTreino = {};
  for (var i = 0; i < arr.length; i++) datasComTreino[arr[i].data] = true;
  var streak = 0;
  var cursor = hojeISO();
  if (!datasComTreino[cursor]) cursor = addDias(cursor, -1);
  while (datasComTreino[cursor]) {
    streak++;
...
function renderInsightTreinoCard() {
  var html = '<div class="card ai-box"><div class="card-title">💡 Insight</div>';
  var linhas = [];
  ['tarcisio', 'aline'].forEach(function (quem) {
    var nomeLabel = quem === 'tarcisio' ? 'Tarcísio' : 'Aline';
    var arr = window.ST[quem].treinos;
    var streak = calcularSequenciaTreino(arr);
    var ultimos7 = 0;
    for (var i = 0; i < arr.length; i++) { var d = diffDias(arr[i].data, hojeISO()); if (d >= 0 && d <= 6) ultimos7++; }
    var frase = nomeLabel + ': ';
    if (streak > 0) frase += streak + ' dia' + (streak > 1 ? 's' : '') + ' seguido' + (streak > 1 ? 's' : '') + ' treinando 🔥';
    else frase += 'sem sequência ativa no momento';
    frase += ' · ' + ultimos7 + ' treino' + (ultimos7 !== 1 ? 's' : '') + ' nos últimos 7 dias';
    linhas.push(frase);
  });
  html += linhas.join('<br><br>');
  html += '</div>';
...
function editarTreino(quem, id) {
  ...
  window._editBuffer.treino = { data: reg.data, inicio: reg.inicio, fim: reg.fim, obs: reg.obs, distancia: reg.distancia, fcMedia: reg.fcMedia, calorias: reg.calorias };
  window._treinoCamposTemp = {};
...
function salvarTreino() {
  ...
  var editando = window._editId.treino;
  var elDist = document.getElementById('treinoDistancia');
  var elFc = document.getElementById('treinoFcMedia');
  var elCal = document.getElementById('treinoCalorias');
  var reg = {
    ...
    distancia: elDist ? (Number(elDist.value) || null) : null,
    fcMedia: elFc ? (Number(elFc.value) || null) : null,
    calorias: elCal ? (Number(elCal.value) || null) : null,
...
function renderIntimidade() {
  ...
  var html = '';
  html += renderInsightIntimidadeCard();
  ...
  html += renderMetaIntimidadeCard();
  html += renderCruzamentoLazerIntimidadeCard();
...
var TIPOS_SAUDE = [
  ...
  { v: 'concentracao', l: '🧠 Concentração' }, { v: 'dor_muscular', l: '💪 Dor muscular' },
  { v: 'outro', l: '➕ Outro' }
];
...
function renderSaude() {
  ...
  html += renderPeriodosCard();
  html += renderGraficoPeriodosCard();
...
var TIPOS_BEBIDA = [
  ...
  { v: 'caipirinha', l: '🍈 Caipirinha' }, { v: 'old_fashioned', l: '🥃 Old Fashioned' },
  { v: 'outro', l: '🫧 Outro' }
];
...
function renderAlcool() {
  ...
  html += renderGraficoBarrasAlcool();
...
MAPA_RERENDER_MODULO.corpo = renderCorpo;

// ===================== CORPO / MEDIDAS =====================
window._corpoFotosExistentes = []; // URLs já salvas (ao editar)
window._corpoFotosNovas = []; // { blob, previewUrl } pendentes de upload
window._corpoCamposTemp = {}; // preserva campos digitados entre re-renders parciais (ex: ao anexar foto)
window._corpoGraficoMetrica = window._corpoGraficoMetrica || 'peso';

function limparFotosCorpoTemp() {
  window._corpoFotosExistentes = [];
  window._corpoFotosNovas = [];
}

var CAMPOS_CORPO_TEXTO = ['corpoPeso', 'corpoAltura', 'corpoCintura', 'corpoQuadril', 'corpoCoxa', 'corpoBraco', 'corpoObs'];

function capturarCamposCorpo() {
  for (var i = 0; i < CAMPOS_CORPO_TEXTO.length; i++) {
    var elC = document.getElementById(CAMPOS_CORPO_TEXTO[i]);
    if (elC) window._corpoCamposTemp[CAMPOS_CORPO_TEXTO[i]] = elC.value;
  }
}
function valorCampoCorpo(id, padrao) {
  if (window._corpoCamposTemp[id] !== undefined) return window._corpoCamposTemp[id];
  return padrao === undefined ? '' : padrao;
}

function renderCorpo() {
  var el = document.getElementById('sec-corpo');
  var editando = window._editId.corpo;
  var html = '';
  html += '<div class="card"><div class="card-title">📏 ' + (editando ? 'Editar medição' : 'Registrar medição') + '</div>';
  html += '<div class="field"><div class="quem-pick">';
  html += '<button type="button" class="quem-btn q-tarcisio' + pillOn('corpoQuem', 'tarcisio') + '" onclick="selectSingle(\'corpoQuem\',\'tarcisio\',this)">👨 Tarcísio</button>';
  html += '<button type="button" class="quem-btn q-aline' + pillOn('corpoQuem', 'aline') + '" onclick="selectSingle(\'corpoQuem\',\'aline\',this)">👩 Aline</button>';
  html += '</div></div>';
  html += '<div class="field"><label>Data</label>' + campoModalHtml('corpoData', 'date', valorPendente('corpoData', eb('corpo', 'data', hojeISO())), 'Data da medição', 'renderCorpo') + '</div>';
  html += '<div class="field"><label>Peso (kg)</label><input class="input" type="number" step="0.1" id="corpoPeso" value="' + valorCampoCorpo('corpoPeso', eb('corpo', 'peso')) + '"></div>';
  html += '<div class="field"><label>Altura (cm) — informe uma vez, será lembrada</label><input class="input" type="number" id="corpoAltura" value="' + valorCampoCorpo('corpoAltura', eb('corpo', 'altura', ultimaAlturaCorpo(window._sel.corpoQuem))) + '"></div>';
  html += '<div class="field"><label>Cintura (cm, opcional)</label><input class="input" type="number" step="0.1" id="corpoCintura" value="' + valorCampoCorpo('corpoCintura', eb('corpo', 'cintura')) + '"></div>';
  html += '<div class="field"><label>Quadril (cm, opcional)</label><input class="input" type="number" step="0.1" id="corpoQuadril" value="' + valorCampoCorpo('corpoQuadril', eb('corpo', 'quadril')) + '"></div>';
  html += '<div class="field"><label>Coxa (cm, opcional)</label><input class="input" type="number" step="0.1" id="corpoCoxa" value="' + valorCampoCorpo('corpoCoxa', eb('corpo', 'coxa')) + '"></div>';
  html += '<div class="field"><label>Braço (cm, opcional)</label><input class="input" type="number" step="0.1" id="corpoBraco" value="' + valorCampoCorpo('corpoBraco', eb('corpo', 'braco')) + '"></div>';

  var totalFotos = window._corpoFotosExistentes.length + window._corpoFotosNovas.length;
  html += '<div class="field"><label>Fotos (até 5, opcional) — ' + totalFotos + '/5</label>';
  if (totalFotos < 5) html += '<input type="file" accept="image/*" id="corpoFotoInput" class="input" onchange="selecionarFotoCorpo(this)">';
  else html += '<div class="li-sub">Limite de 5 fotos atingido. Remova uma para adicionar outra.</div>';
  if (totalFotos > 0) {
    html += '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px;">';
    for (var e = 0; e < window._corpoFotosExistentes.length; e++) {
      html += '<div style="position:relative;"><img src="' + window._corpoFotosExistentes[e] + '" style="width:90px;height:90px;object-fit:cover;border-radius:10px;">';
      html += '<button type="button" onclick="removerFotoExistenteCorpo(' + e + ')" style="position:absolute;top:-6px;right:-6px;background:var(--red);color:#fff;border:none;border-radius:50%;width:22px;height:22px;">✕</button></div>';
    }
    for (var n = 0; n < window._corpoFotosNovas.length; n++) {
      html += '<div style="position:relative;"><img src="' + window._corpoFotosNovas[n].previewUrl + '" style="width:90px;height:90px;object-fit:cover;border-radius:10px;">';
      html += '<button type="button" onclick="removerFotoNovaCorpo(' + n + ')" style="position:absolute;top:-6px;right:-6px;background:var(--red);color:#fff;border:none;border-radius:50%;width:22px;height:22px;">✕</button></div>';
    }
    html += '</div>';
  }
  html += '</div>';
  html += '<div class="field"><label>Observação (opcional)</label><textarea class="input" id="corpoObs">' + valorCampoCorpo('corpoObs', eb('corpo', 'obs')) + '</textarea></div>';
  html += '<button class="btn btn-primary" onclick="salvarCorpo()">' + (editando ? 'Salvar edição' : 'Salvar medição') + '</button>';
  if (editando) html += ' <button class="btn btn-ghost" onclick="cancelarEdicaoCorpo()" style="margin-top:8px;">Cancelar edição</button>';
  html += '</div>';

  html += renderInsightPesoCard();
  html += renderMetaPesoCard();
  html += renderGraficoCorpoCard();
  html += '<div class="card"><div class="card-title">Histórico</div><div id="corpoLista"></div></div>';
  el.innerHTML = html;
  renderListaCorpo();
}

function ultimaAlturaCorpo(quem) {
  if (!quem || !window.ST.corpo[quem]) return undefined;
  var arr = window.ST.corpo[quem].slice().sort(function (a, b) { return a.data < b.data ? 1 : -1; });
  for (var i = 0; i < arr.length; i++) if (arr[i].altura) return arr[i].altura;
  return undefined;
}

function selecionarFotoCorpo(input) {
  if (!input.files || !input.files[0]) return;
  var totalAtual = window._corpoFotosExistentes.length + window._corpoFotosNovas.length;
  if (totalAtual >= 5) { alert('Limite de 5 fotos por registro.'); return; }
  comprimirImagem(input.files[0], function (blob) {
    window._corpoFotosNovas.push({ blob: blob, previewUrl: URL.createObjectURL(blob) });
    capturarCamposCorpo();
    renderCorpo();
  });
}
window.selecionarFotoCorpo = selecionarFotoCorpo;

function removerFotoExistenteCorpo(idx) {
  window._corpoFotosExistentes.splice(idx, 1);
  capturarCamposCorpo();
  renderCorpo();
}
window.removerFotoExistenteCorpo = removerFotoExistenteCorpo;

function removerFotoNovaCorpo(idx) {
  window._corpoFotosNovas.splice(idx, 1);
  capturarCamposCorpo();
  renderCorpo();
}
window.removerFotoNovaCorpo = removerFotoNovaCorpo;

function setGraficoCorpoMetrica(m) {
  window._corpoGraficoMetrica = m;
  capturarCamposCorpo();
  renderCorpo();
}
window.setGraficoCorpoMetrica = setGraficoCorpoMetrica;

function renderInsightPesoCard() {
  var html = '<div class="card ai-box"><div class="card-title">💡 Insight</div>';
  var linhas = [];
  ['tarcisio', 'aline'].forEach(function (quem) {
    var nomeLabel = quem === 'tarcisio' ? 'Tarcísio' : 'Aline';
    ...
    var atual = arr[arr.length - 1];
    var meta = window.ST.corpo.metas[quem];
    var frase = nomeLabel + ': ';
    if (meta) {
      var diff = atual.peso - meta;
      if (Math.abs(diff) < 0.05) frase += 'está exatamente na meta de peso (' + meta + 'kg). 🎉';
      else frase += 'está a ' + Math.abs(diff).toFixed(1) + 'kg da meta (' + meta + 'kg).';
    } else {
      frase += 'ainda não definiu uma meta de peso.';
    }
    if (arr.length >= 2) {
      var anterior = arr[arr.length - 2];
      var evolucao = atual.peso - anterior.peso;
      if (Math.abs(evolucao) < 0.05) frase += ' Peso estável desde a última medição (' + fmtDate(anterior.data) + ').';
      else frase += ' Desde a última medição (' + fmtDate(anterior.data) + '), ' + (evolucao < 0 ? 'perdeu ' : 'ganhou ') + Math.abs(evolucao).toFixed(1) + 'kg.';
    }
    linhas.push(frase);
  });
  html += linhas.length ? linhas.join('<br><br>') : 'Registre pelo menos uma medição para ver o insight.';
  html += '</div>';
  return html;
}

function renderMetaPesoCard() {
  var html = '<div class="card"><div class="card-title">🎯 Meta de peso</div>';
  ['tarcisio', 'aline'].forEach(function (quem) {
    var nomeLabel = quem === 'tarcisio' ? '👨 Tarcísio' : '👩 Aline';
    var arr = window.ST.corpo[quem].slice().sort(function (a, b) { return a.data < b.data ? -1 : 1; });
    var pesoAtual = arr.length ? arr[arr.length - 1].peso : null;
    var pesoInicial = arr.length ? arr[0].peso : null;
    var meta = window.ST.corpo.metas[quem];
    html += '<div class="card-sub" style="font-weight:700;color:var(--txt);margin-top:10px;">' + nomeLabel + '</div>';
    html += '<div class="row2">';
    html += '<div class="field"><label>Meta de peso (kg)</label><input class="input" type="number" step="0.1" id="metaPeso_' + quem + '" value="' + (meta !== null && meta !== undefined ? meta : '') + '"></div>';
    html += '<div class="field" style="display:flex;align-items:flex-end;"><button type="button" class="btn btn-ghost" style="width:100%;" onclick="salvarMetaPeso(\'' + quem + '\')">Salvar meta</button></div>';
    html += '</div>';
    if (meta && pesoAtual) {
      var diferenca = pesoAtual - meta;
      if (Math.abs(diferenca) < 0.05) {
        html += '<div class="li-sub" style="color:#2D6A4F;font-weight:700;">🎉 Meta atingida!</div>';
      } else {
        var direcao = diferenca > 0 ? 'perder' : 'ganhar';
        html += '<div class="li-sub">Peso atual: ' + pesoAtual + 'kg · Faltam <b>' + Math.abs(diferenca).toFixed(1) + 'kg</b> para ' + direcao + '</div>';
        var progresso = 0;
        if (pesoInicial !== null && pesoInicial !== meta) progresso = ((pesoInicial - pesoAtual) / (pesoInicial - meta)) * 100;
        var progressoClamp = Math.max(0, Math.min(100, progresso));
        html += '<div style="background:var(--border);border-radius:8px;height:10px;overflow:hidden;margin-top:6px;"><div style="background:var(--primary);height:100%;width:' + progressoClamp + '%;"></div></div>';
        html += '<div class="li-sub" style="margin-top:4px;">' + Math.round(progresso) + '% do caminho percorrido</div>';
      }
    } else if (meta) {
      html += '<div class="li-sub">Registre um peso para ver o progresso.</div>';
    }
  });
  html += '</div>';
  return html;
...
function salvarMetaPeso(quem) {
  var val = Number(document.getElementById('metaPeso_' + quem).value) || null;
  window.ST.corpo.metas[quem] = val;
  window.saveState();
  renderCorpo();
}
window.salvarMetaPeso = salvarMetaPeso;

function renderGraficoCorpoCard() {
  var metrica = window._corpoGraficoMetrica;
  var opcoes = [['peso', 'Peso'], ['imc', 'IMC'], ['cintura', 'Cintura'], ['quadril', 'Quadril'], ['coxa', 'Coxa'], ['braco', 'Braço'], ['rcq', 'RCQ'], ['rcest', 'RCEst']];
  var html = '<div class="card"><div class="card-title" style="display:flex;align-items:center;justify-content:space-between;">📈 Evolução <button type="button" onclick="abrirModalReferencia()" style="border:none;background:var(--primary-x);color:var(--primary);width:26px;height:26px;border-radius:50%;font-weight:700;font-size:14px;">ⓘ</button></div>';
  html += '<div class="pill-row" style="margin-bottom:10px;">';
  for (var i = 0; i < opcoes.length; i++) {
    html += '<button type="button" class="tipo-pill' + (metrica === opcoes[i][0] ? ' on' : '') + '" onclick="setGraficoCorpoMetrica(\'' + opcoes[i][0] + '\')">' + opcoes[i][1] + '</button>';
  }
  html += '</div>';
  ...
  ['tarcisio', 'aline'].forEach(function (quem) {
    ...
    for (var i = 0; i < arr.length; i++) {
      var v = null;
      var alturaSuspeitaG = arr[i].altura && (arr[i].altura < 100 || arr[i].altura > 230);
      if (metrica === 'imc') v = alturaSuspeitaG ? null : calcIMC(arr[i].peso, arr[i].altura);
      else if (metrica === 'rcq') v = calcRCQ(arr[i].cintura, arr[i].quadril);
      else if (metrica === 'rcest') v = alturaSuspeitaG ? null : calcRCEst(arr[i].cintura, arr[i].altura);
      else v = arr[i][metrica];
      if (v !== null && v !== undefined && v !== '') pontos.push({ x: arr[i].data, y: Number(v) });
    }
    var cor = quem === 'tarcisio' ? '#1A6B9A' : '#C2185B';
    var nomeLabel = quem === 'tarcisio' ? '👨 Tarcísio' : '👩 Aline';
    html += '<div class="card-sub" style="font-weight:700;color:var(--txt);margin-top:8px;">' + nomeLabel + '</div>';
    html += renderGraficoLinha(pontos, cor);
  });
  html += '</div>';
  return html;
}
...
function renderListaCorpo() {
  var todos = [];
  ['tarcisio', 'aline'].forEach(function (quem) {
    for (var i = 0; i < window.ST.corpo[quem].length; i++) {
      var r = window.ST.corpo[quem][i]; r._quem = quem; todos.push(r);
    }
  });
  todos.sort(function (a, b) { return a.data < b.data ? 1 : -1; });
  var html = '';
  if (todos.length === 0) html = '<div class="li-sub">Nenhuma medição registrada.</div>';
  for (var k = 0; k < todos.length; k++) {
    var reg = todos[k];
    var quemLabel = reg._quem === 'tarcisio' ? '👨 Tarcísio' : '👩 Aline';
    var alturaSuspeita = reg.altura && (reg.altura < 100 || reg.altura > 230);
    var imc = alturaSuspeita ? null : calcIMC(reg.peso, reg.altura);
    var rcq = calcRCQ(reg.cintura, reg.quadril);
    var rcest = alturaSuspeita ? null : calcRCEst(reg.cintura, reg.altura);
    html += '<div class="list-item"><div>';
    html += '<div class="li-main">' + quemLabel + ' — ' + fmtDate(reg.data) + '</div>';
    html += '<div class="li-sub">Peso: ' + (reg.peso || '—') + 'kg' + (reg.altura ? ' · Altura: ' + reg.altura + 'cm' : '') + (imc ? ' · IMC: ' + fmt1(imc) + ' (' + classificarIMC(imc, reg._quem) + ')' : '') + '</div>';
    if (alturaSuspeita) html += '<div class="li-sub" style="color:var(--red);">⚠️ Altura registrada (' + reg.altura + 'cm) parece incorreta — edite este registro para corrigir.</div>';
    if (reg.cintura || reg.quadril || reg.coxa || reg.braco) {
      html += '<div class="li-sub">' + (reg.cintura ? 'Cintura: ' + reg.cintura + 'cm ' : '') + (reg.quadril ? '· Quadril: ' + reg.quadril + 'cm ' : '') + (reg.coxa ? '· Coxa: ' + reg.coxa + 'cm ' : '') + (reg.braco ? '· Braço: ' + reg.braco + 'cm' : '') + '</div>';
    }
    if (rcq) html += '<div class="li-sub">RCQ: ' + rcq.toFixed(2) + ' (' + classificarRCQ(rcq, reg._quem) + ')</div>';
    if (rcest) html += '<div class="li-sub">RCEst: ' + rcest.toFixed(2) + ' (' + classificarRCEst(rcest, reg._quem) + ')</div>';
    var fotosReg = reg.fotos && reg.fotos.length ? reg.fotos : (reg.fotoUrl ? [reg.fotoUrl] : []);
    if (fotosReg.length > 0) {
      html += '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:6px;">';
      for (var fi = 0; fi < fotosReg.length; fi++) html += '<img src="' + fotosReg[fi] + '" style="width:80px;height:80px;object-fit:cover;border-radius:8px;cursor:pointer;" onclick="abrirFotoAmpliada(\'' + fotosReg[fi] + '\')">';
      html += '</div>';
    }
    if (reg.obs) html += '<div class="li-sub">' + esc(reg.obs) + '</div>';
    html += '</div><div class="li-actions"><button onclick="editarCorpo(\'' + reg._quem + '\',' + reg.id + ')">✏️</button><button onclick="excluirCorpo(\'' + reg._quem + '\',' + reg.id + ')">🗑</button></div></div>';
  }
  document.getElementById('corpoLista').innerHTML = html;
}

function editarCorpo(quem, id) {
  var arr = window.ST.corpo[quem];
  var reg = null;
  for (var i = 0; i < arr.length; i++) if (arr[i].id === id) { reg = arr[i]; break; }
  if (!reg) return;
  window._sel = {};
  window._sel.corpoQuem = quem;
  window._editId.corpo = { quem: quem, id: id };
  window._editBuffer.corpo = { data: reg.data, peso: reg.peso, altura: reg.altura, cintura: reg.cintura, quadril: reg.quadril, coxa: reg.coxa, braco: reg.braco, obs: reg.obs };
  window._corpoCamposTemp = {};
  window._corpoFotosExistentes = (reg.fotos && reg.fotos.length ? reg.fotos.slice() : (reg.fotoUrl ? [reg.fotoUrl] : []));
  window._corpoFotosNovas = [];
  limparValoresPendentes(['corpoData']);
  renderCorpo();
}
window.editarCorpo = editarCorpo;

function cancelarEdicaoCorpo() {
  window._editId.corpo = null;
  window._editBuffer.corpo = null;
  window._corpoCamposTemp = {};
  limparFotosCorpoTemp();
  limparValoresPendentes(['corpoData']);
  renderCorpo();
}
window.cancelarEdicaoCorpo = cancelarEdicaoCorpo;

async function salvarCorpo() {
  var quem = window._sel.corpoQuem;
  if (!quem) { alert('Selecione quem'); return; }
  var peso = Number(document.getElementById('corpoPeso').value) || null;
  if (!peso) { alert('Preencha o peso'); return; }
  var alturaDigitada = Number(document.getElementById('corpoAltura').value) || null;
  if (alturaDigitada && alturaDigitada < 3) alturaDigitada = alturaDigitada * 100; // corrige quando a pessoa digita em metros (ex: 1.78) num campo que espera cm
  if (alturaDigitada && (alturaDigitada < 100 || alturaDigitada > 230)) {
    if (!confirm('A altura ficou em ' + alturaDigitada + 'cm, o que parece incomum. Confirma que está correto?')) return;
  }
  var editando = window._editId.corpo;
  var reg = {
    id: editando ? editando.id : uid(),
    data: valorPendente('corpoData', eb('corpo', 'data', hojeISO())) || hojeISO(),
    peso: peso,
    altura: alturaDigitada,
    cintura: Number(document.getElementById('corpoCintura').value) || null,
    quadril: Number(document.getElementById('corpoQuadril').value) || null,
    coxa: Number(document.getElementById('corpoCoxa').value) || null,
    braco: Number(document.getElementById('corpoBraco').value) || null,
    fotos: window._corpoFotosExistentes.slice(),
    obs: document.getElementById('corpoObs').value || '',
    registradoPor: window._userName || null
  };

  if (window._corpoFotosNovas.length > 0) {
    for (var f = 0; f < window._corpoFotosNovas.length; f++) {
      try {
        var caminho = 'corpo/' + quem + '/' + reg.id + '_' + f + '_' + uid() + '.jpg';
        var url = await window._uploadFoto(window._corpoFotosNovas[f].blob, caminho);
        reg.fotos.push(url);
      } catch (e) {
        alert('Não foi possível enviar uma das fotos, mas o resto do registro será salvo. Tente reenviar editando este registro.');
      }
    }
  }

  if (editando) {
    var arrAntigo = window.ST.corpo[editando.quem];
    for (var i = 0; i < arrAntigo.length; i++) if (arrAntigo[i].id === editando.id) { arrAntigo.splice(i, 1); break; }
    window._editId.corpo = null;
    window._editBuffer.corpo = null;
  }
  window.ST.corpo[quem].push(reg);
  window.saveState();
  window._corpoCamposTemp = {};
  limparFotosCorpoTemp();
  limparValoresPendentes(['corpoData']);
...
window.salvarCorpo = salvarCorpo;

function excluirCorpo(quem, id) {
...
window.excluirCorpo = excluirCorpo;

// ===================== PERÍODOS DE USO CONTÍNUO (remédio ou atividade) =====================
var TIPOS_REMEDIO_CONTINUO = ['Antidepressivo', 'Emagrecimento', 'Queda de cabelo', 'Reposição de vitaminas'];
var TIPOS_SUPLEMENTO_CONTINUO = ['Whey Protein', 'Creatina', 'Ômega 3', 'Vitamina D', 'Multivitamínico'];

window._periodoCamposTemp = {};
var CAMPOS_PERIODO_TEXTO = ['periodoNome', 'periodoObs'];
function capturarCamposPeriodo() {
  for (var i = 0; i < CAMPOS_PERIODO_TEXTO.length; i++) {
    var elP = document.getElementById(CAMPOS_PERIODO_TEXTO[i]);
    if (elP) window._periodoCamposTemp[CAMPOS_PERIODO_TEXTO[i]] = elP.value;
  }
}
window.capturarCamposPeriodo = capturarCamposPeriodo;
function valorCampoPeriodo(id, padrao) {
  if (window._periodoCamposTemp[id] !== undefined) return window._periodoCamposTemp[id];
  return padrao === undefined ? '' : padrao;
}
function renderSaudeComCapturaPeriodo() {
  capturarCamposPeriodo();
  renderSaude();
}
window.renderSaudeComCapturaPeriodo = renderSaudeComCapturaPeriodo;

function renderPeriodosCard() {
  var editando = window._editId.periodo;
  var tipoAtual = window._sel.periodoTipo || 'remedio';
  var html = '<div class="card"><div class="card-title">💊 ' + (editando ? 'Editar uso contínuo' : 'Registrar uso contínuo') + '</div>';
  html += '<div class="card-sub">Para remédio ou suplemento que você usa por um período (ex: antidepressivo, whey protein)</div>';
  html += '<div class="field"><div class="quem-pick">';
  html += '<button type="button" class="quem-btn q-tarcisio' + pillOn('periodoQuem', 'tarcisio') + '" onclick="selectSingle(\'periodoQuem\',\'tarcisio\',this)">👨 Tarcísio</button>';
  html += '<button type="button" class="quem-btn q-aline' + pillOn('periodoQuem', 'aline') + '" onclick="selectSingle(\'periodoQuem\',\'aline\',this)">👩 Aline</button>';
  html += '</div></div>';
  html += '<div class="field"><label>Tipo</label><div class="pill-row">';
  html += '<button type="button" class="tipo-pill' + pillOn('periodoTipo', 'remedio') + '" onclick="capturarCamposPeriodo();selectSingle(\'periodoTipo\',\'remedio\',this);renderSaude();">💊 Remédio</button>';
  html += '<button type="button" class="tipo-pill' + pillOn('periodoTipo', 'suplemento') + '" onclick="capturarCamposPeriodo();selectSingle(\'periodoTipo\',\'suplemento\',this);renderSaude();">🧴 Suplemento</button>';
  html += '</div></div>';
  if (tipoAtual === 'remedio') {
    html += '<div class="field"><label>Comuns (toque para preencher)</label><div class="pill-row">';
    for (var i = 0; i < TIPOS_REMEDIO_CONTINUO.length; i++) html += '<button type="button" class="tipo-pill" onclick="preencherPeriodoNome(\'' + TIPOS_REMEDIO_CONTINUO[i] + '\')">' + TIPOS_REMEDIO_CONTINUO[i] + '</button>';
    html += '</div></div>';
  } else if (tipoAtual === 'suplemento') {
    html += '<div class="field"><label>Comuns (toque para preencher)</label><div class="pill-row">';
    for (var si = 0; si < TIPOS_SUPLEMENTO_CONTINUO.length; si++) html += '<button type="button" class="tipo-pill" onclick="preencherPeriodoNome(\'' + TIPOS_SUPLEMENTO_CONTINUO[si] + '\')">' + TIPOS_SUPLEMENTO_CONTINUO[si] + '</button>';
    html += '</div></div>';
  }
  html += '<div class="field"><label>Nome</label><input class="input" id="periodoNome" placeholder="Ex: Sertralina 50mg" value="' + valorCampoPeriodo('periodoNome', eb('periodo', 'nome')) + '"></div>';
  html += '<div class="field"><label>Início</label>' + campoModalHtml('periodoDataInicio', 'date', valorPendente('periodoDataInicio', eb('periodo', 'dataInicio', hojeISO())), 'Início do uso', 'renderSaudeComCapturaPeriodo') + '</div>';
  html += '<div class="field"><label>Fim (deixe vazio se ainda está em uso)</label>' + campoModalHtml('periodoDataFim', 'date', valorPendente('periodoDataFim', eb('periodo', 'dataFim')), 'Fim do uso', 'renderSaudeComCapturaPeriodo') + '</div>';
  html += '<div class="field"><label>Observação (opcional)</label><textarea class="input" id="periodoObs">' + valorCampoPeriodo('periodoObs', eb('periodo', 'obs')) + '</textarea></div>';
  html += '<button class="btn btn-primary" onclick="salvarPeriodo()">' + (editando ? 'Salvar edição' : 'Salvar') + '</button>';
  if (editando) html += ' <button class="btn btn-ghost" onclick="cancelarEdicaoPeriodo()" style="margin-top:8px;">Cancelar edição</button>';
  html += '</div>';
  html += '<div class="card"><div class="card-title">Usos contínuos registrados</div><div id="periodosLista"></div></div>';
  return html;
}

function preencherPeriodoNome(nome) {
  document.getElementById('periodoNome').value = nome;
}
window.preencherPeriodoNome = preencherPeriodoNome;

function renderListaPeriodos() {
  var elL = document.getElementById('periodosLista');
  if (!elL) return;
  var lista = window.ST.periodos.lista.slice().sort(function (a, b) { return a.dataInicio < b.dataInicio ? 1 : -1; });
  var html = '';
  if (lista.length === 0) html = '<div class="li-sub">Nenhum uso contínuo registrado.</div>';
  for (var i = 0; i < lista.length; i++) {
    var p = lista[i];
    var quemLabel = p.quem === 'tarcisio' ? '👨 Tarcísio' : '👩 Aline';
    var emUso = !p.dataFim;
    html += '<div class="list-item"><div>';
    html += '<div class="li-main">' + (p.tipo === 'remedio' ? '💊' : '🧴') + ' ' + esc(p.nome) + ' — ' + quemLabel + '</div>';
    html += '<div class="li-sub">' + fmtDate(p.dataInicio) + (p.dataFim ? ' → ' + fmtDate(p.dataFim) : '') + ' · <span style="font-weight:700;color:' + (emUso ? '#2D6A4F' : 'var(--muted)') + ';">' + (emUso ? 'Em uso' : 'Concluído') + '</span></div>';
    if (p.obs) html += '<div class="li-sub">' + esc(p.obs) + '</div>';
    html += '</div><div class="li-actions"><button onclick="editarPeriodo(' + p.id + ')">✏️</button><button onclick="excluirPeriodo(' + p.id + ')">🗑</button></div></div>';
  }
  elL.innerHTML = html;
}

function editarPeriodo(id) {
  var arr = window.ST.periodos.lista;
  var p = null;
  for (var i = 0; i < arr.length; i++) if (arr[i].id === id) { p = arr[i]; break; }
  if (!p) return;
  window._sel.periodoQuem = p.quem;
  window._sel.periodoTipo = p.tipo;
  window._editId.periodo = { id: id };
  window._editBuffer.periodo = { nome: p.nome, dataInicio: p.dataInicio, dataFim: p.dataFim, obs: p.obs };
  window._periodoCamposTemp = {};
  limparValoresPendentes(['periodoDataInicio', 'periodoDataFim']);
  renderSaude();
}
window.editarPeriodo = editarPeriodo;

function cancelarEdicaoPeriodo() {
  window._editId.periodo = null;
  window._editBuffer.periodo = null;
  window._periodoCamposTemp = {};
  limparValoresPendentes(['periodoDataInicio', 'periodoDataFim']);
  renderSaude();
}
window.cancelarEdicaoPeriodo = cancelarEdicaoPeriodo;

function salvarPeriodo() {
  var quem = window._sel.periodoQuem;
  var tipo = window._sel.periodoTipo || 'remedio';
  var nome = document.getElementById('periodoNome').value;
  if (!quem) { alert('Selecione quem'); return; }
  if (!nome) { alert('Preencha o nome'); return; }
  var dataInicio = valorPendente('periodoDataInicio', eb('periodo', 'dataInicio', hojeISO())) || hojeISO();
  var dataFim = valorPendente('periodoDataFim', eb('periodo', 'dataFim')) || null;
  if (dataFim && dataFim < dataInicio) { alert('A data de fim não pode ser antes do início'); return; }
  var editando = window._editId.periodo;
  var reg = {
    id: editando ? editando.id : uid(), quem: quem, tipo: tipo, nome: nome,
  ...
  window.ST.periodos.lista.push(reg);
  window.saveState();
  window._periodoCamposTemp = {};
  limparValoresPendentes(['periodoDataInicio', 'periodoDataFim']);
  renderSaude();
...
function excluirPeriodo(id) {
  if (!confirm('Excluir este registro de uso contínuo?')) return;
  var arr = window.ST.periodos.lista;
  for (var i = 0; i < arr.length; i++) if (arr[i].id === id) { arr.splice(i, 1); break; }
  window.saveState();
  renderSaude();
...
// ===================== MODAL DE REFERÊNCIA (IMC, RCQ, RCEst) =====================
function abrirModalReferencia() {
  var html = '';
  html += '<div class="card-sub" style="font-weight:700;color:var(--txt);">IMC (Índice de Massa Corporal)</div>';
  html += '<table class="tbl"><tr><th>IMC</th><th>Classificação</th></tr>';
  html += '<tr><td>&lt; 18,5</td><td>Abaixo do peso</td></tr>';
  html += '<tr><td>18,5 – 24,9</td><td>Peso normal</td></tr>';
  html += '<tr><td>25 – 29,9</td><td>Sobrepeso</td></tr>';
  html += '<tr><td>≥ 30</td><td>Obesidade</td></tr></table>';
  html += '<div class="li-sub" style="margin-top:6px;">A partir de 65 anos, o teto de "peso normal" sobe para 27 (diretriz geriátrica).</div>';

  html += '<div class="card-sub" style="font-weight:700;color:var(--txt);margin-top:16px;">RCQ (Relação Cintura-Quadril)</div>';
  html += '<table class="tbl"><tr><th>Sexo</th><th>Baixo risco</th><th>Risco aumentado</th></tr>';
  html += '<tr><td>Homens</td><td>≤ 0,90</td><td>&gt; 0,90</td></tr>';
  html += '<tr><td>Mulheres</td><td>≤ 0,85</td><td>&gt; 0,85</td></tr></table>';
  html += '<div class="li-sub" style="margin-top:6px;">Referência OMS. Sem diferenciação por idade.</div>';

  html += '<div class="card-sub" style="font-weight:700;color:var(--txt);margin-top:16px;">RCEst (Relação Cintura-Estatura)</div>';
  html += '<table class="tbl"><tr><th>Idade</th><th>Baixo risco</th><th>Risco aumentado</th></tr>';
  html += '<tr><td>Até 50 anos</td><td>&lt; 0,50</td><td>≥ 0,50</td></tr>';
  html += '<tr><td>50+ anos</td><td>&lt; 0,60</td><td>≥ 0,60</td></tr></table>';
  html += '<div class="li-sub" style="margin-top:6px;">Regra prática: manter a cintura abaixo de metade da altura. Sem diferenciação por sexo.</div>';

  html += '<div class="li-sub" style="margin-top:16px;font-style:italic;">Essas tabelas são referências gerais de saúde pública, não substituem avaliação médica individual.</div>';

  document.getElementById('modalReferenciaConteudo').innerHTML = html;
  abrirModal('modal-referencia');
}
window.abrirModalReferencia = abrirModalReferencia;

// ===================== INSIGHT: SAÚDE =====================
function renderInsightSaudeCard() {
  var linhas = [];
  ['tarcisio', 'aline'].forEach(function (quem) {
    var nomeLabel = quem === 'tarcisio' ? 'Tarcísio' : 'Aline';
    var arr = window.ST[quem].doencas;
    var esteMes = 0, mesAnterior = 0;
    for (var i = 0; i < arr.length; i++) {
      if (arr[i].tipo === 'concentracao') continue;
      var d = diffDias(arr[i].inicio, hojeISO());
      if (d >= 0 && d <= 29) esteMes++;
      else if (d >= 30 && d <= 59) mesAnterior++;
    }
    var frase = nomeLabel + ': ' + esteMes + ' episódio(s) nos últimos 30 dias';
    if (mesAnterior > 0 || esteMes > 0) {
      if (esteMes < mesAnterior) frase += ', menos que no mês anterior (' + mesAnterior + ').';
      else if (esteMes > mesAnterior) frase += ', mais que no mês anterior (' + mesAnterior + ').';
      else frase += ', igual ao mês anterior.';
    } else {
      frase += '.';
    }
    linhas.push(frase);
  });
  var emUso = window.ST.periodos.lista.filter(function (p) { return !p.dataFim; }).length;
  linhas.push(emUso + ' uso(s) contínuo(s) ativo(s) no momento.');
  return '<div class="card ai-box"><div class="card-title">💡 Insight</div>' + linhas.join('<br><br>') + '</div>';
}

// ===================== INSIGHT: CICLO =====================
function renderInsightCicloCard() {
  var ciclos = window.ST.aline.ciclos;
  if (ciclos.length === 0) return '<div class="card ai-box"><div class="card-title">💡 Insight</div>Registre pelo menos um ciclo para ver o insight.</div>';
  var ord = ciclosOrdenados(ciclos);
  var ultimo = ord[ord.length - 1];
  var duracaoMedia = estimarDuracaoMedia(ciclos);
  var frase = 'Duração média histórica do ciclo: ' + duracaoMedia + ' dias (baseado em ' + Math.max(ord.length - 1, 0) + ' ciclo(s) completo(s)).';
  var previsao = addDias(ultimo.dataInicio, duracaoMedia);
  var diasAtraso = diffDias(previsao, hojeISO());
  if (diasAtraso > 0) frase += ' A menstruação está ' + diasAtraso + ' dia(s) além do previsto (esperada em ' + fmtDate(previsao) + ').';
  else frase += ' Próxima menstruação prevista para ' + fmtDate(previsao) + '.';
  return '<div class="card ai-box"><div class="card-title">💡 Insight</div>' + frase + '</div>';
}

// ===================== INSIGHT: ÁLCOOL (com referência de consumo moderado) =====================
function classificarAlcoolSemana(doses, quem) {
  var limite = quem === 'aline' ? 7 : 14;
  return doses <= limite ? 'Moderado' : 'Consumo excessivo';
}
function houveBingeNaSemana(arr, quem) {
  var limiteBinge = quem === 'aline' ? 4 : 5;
  for (var i = 0; i < arr.length; i++) {
    var d = diffDias(arr[i].data, hojeISO());
    if (d >= 0 && d <= 6 && (arr[i].quantidade || 0) >= limiteBinge) return true;
  }
  return false;
}

function renderInsightAlcoolCard() {
  var linhas = [];
  ['tarcisio', 'aline'].forEach(function (quem) {
    var nomeLabel = quem === 'tarcisio' ? 'Tarcísio' : 'Aline';
    var arr = window.ST[quem].alcool;
    var doses7 = 0, semanasAnteriores = [0, 0, 0, 0];
    for (var i = 0; i < arr.length; i++) {
      var d = diffDias(arr[i].data, hojeISO());
      if (d >= 0 && d <= 6) doses7 += (arr[i].quantidade || 0);
      else if (d >= 7 && d <= 34) {
        var semanaIdx = Math.floor((d - 7) / 7);
        if (semanaIdx >= 0 && semanaIdx < 4) semanasAnteriores[semanaIdx] += (arr[i].quantidade || 0);
      }
    }
    var mediaAnterior = media(semanasAnteriores);
    var classificacao = classificarAlcoolSemana(doses7, quem);
    var frase = nomeLabel + ': ' + doses7 + ' dose(s) nos últimos 7 dias — ' + classificacao + '.';
    if (mediaAnterior !== null) {
      if (doses7 < mediaAnterior - 0.5) frase += ' Abaixo da média das últimas semanas (' + fmt1(mediaAnterior) + ').';
      else if (doses7 > mediaAnterior + 0.5) frase += ' Acima da média das últimas semanas (' + fmt1(mediaAnterior) + ').';
    }
    if (houveBingeNaSemana(arr, quem)) frase += ' ⚠️ Consumo elevado em um único dia nos últimos 7 dias.';
    linhas.push(frase);
  });
  var titulo = '<div class="card-title" style="display:flex;align-items:center;justify-content:space-between;">💡 Insight <button type="button" onclick="abrirModalReferenciaAlcool()" style="border:none;background:var(--primary-x);color:var(--primary);width:26px;height:26px;border-radius:50%;font-weight:700;font-size:14px;">ⓘ</button></div>';
  return '<div class="card ai-box">' + titulo + linhas.join('<br><br>') + '</div>';
}

function abrirModalReferenciaAlcool() {
  var html = '';
  html += '<div class="card-sub" style="font-weight:700;color:var(--txt);">Consumo semanal (doses)</div>';
  html += '<table class="tbl"><tr><th>Sexo</th><th>Moderado</th><th>Excessivo</th></tr>';
  html += '<tr><td>Homens</td><td>até 14/semana</td><td>≥ 15/semana</td></tr>';
  html += '<tr><td>Mulheres</td><td>até 7/semana</td><td>≥ 8/semana</td></tr></table>';
  html += '<div class="card-sub" style="font-weight:700;color:var(--txt);margin-top:16px;">Consumo elevado em um único dia (binge)</div>';
  html += '<table class="tbl"><tr><th>Sexo</th><th>Limite</th></tr>';
  html += '<tr><td>Homens</td><td>5+ doses em ~2h</td></tr>';
  html += '<tr><td>Mulheres</td><td>4+ doses em ~2h</td></tr></table>';
  html += '<div class="li-sub" style="margin-top:12px;">Referência: NIAAA/CDC. A OMS considera que nenhuma quantidade de álcool é totalmente livre de risco — essas faixas indicam nível de risco relativo, não um "seguro".</div>';
  document.getElementById('modalReferenciaConteudo').innerHTML = html;
  abrirModal('modal-referencia');
}
window.abrirModalReferenciaAlcool = abrirModalReferenciaAlcool;

// ===================== INSIGHT: LAZER =====================
function renderInsightLazerCard() {
  var arr = window.ST.juntos.momentos.filter(function (m) { return m.quem === 'casal'; }).slice().sort(function (a, b) { return a.data < b.data ? 1 : -1; });
  var frase;
  if (arr.length === 0) {
    frase = 'Nenhum programa a dois (Casal) registrado ainda.';
  } else {
    var ultimo = arr[0];
    var dias = diffDias(ultimo.data, hojeISO());
    frase = dias === 0 ? 'Programa a dois hoje mesmo (' + tipoJuntosLabel(ultimo.tipo) + ')! 🎉' : 'Já se passaram ' + dias + ' dia(s) desde o último programa a dois (' + tipoJuntosLabel(ultimo.tipo) + ', ' + fmtDate(ultimo.data) + ').';
  }
  return '<div class="card ai-box"><div class="card-title">💡 Insight</div>' + frase + '</div>';
}

// ===================== INSIGHT + META + CRUZAMENTO: INTIMIDADE =====================
function renderInsightIntimidadeCard() {
  var arr = window.ST.intimidade.encontros;
  var tent7 = 0, cons7 = 0, tentAnt = 0, consAnt = 0;
  for (var i = 0; i < arr.length; i++) {
    var d = diffDias(arr[i].data, hojeISO());
    if (d >= 0 && d <= 6) { tent7++; if (arr[i].consumado === 'sim') cons7++; }
    else if (d >= 7 && d <= 13) { tentAnt++; if (arr[i].consumado === 'sim') consAnt++; }
  }
  var frase = tent7 + ' tentativa(s) nos últimos 7 dias (' + cons7 + ' consumada' + (cons7 !== 1 ? 's' : '') + ')';
  if (tentAnt > 0 || tent7 > 0) {
    if (tent7 > tentAnt) frase += ', mais tentativas que na semana anterior (' + tentAnt + ').';
    else if (tent7 < tentAnt) frase += ', menos tentativas que na semana anterior (' + tentAnt + ').';
    else frase += ', mesma quantidade de tentativas que na semana anterior.';
  } else {
    frase += '.';
  }

  var taxa7 = tent7 > 0 ? (cons7 / tent7 * 100) : null;
  var taxaAnt = tentAnt > 0 ? (consAnt / tentAnt * 100) : null;
  var fraseTaxa;
  if (taxa7 !== null) {
    fraseTaxa = 'Taxa de sucesso: ' + Math.round(taxa7) + '%';
    if (taxaAnt !== null) {
      var diffTaxa = taxa7 - taxaAnt;
      if (Math.abs(diffTaxa) < 5) fraseTaxa += ', estável em relação à semana anterior (' + Math.round(taxaAnt) + '%).';
      else if (diffTaxa > 0) fraseTaxa += ', subindo em relação à semana anterior (' + Math.round(taxaAnt) + '%).';
      else fraseTaxa += ', caindo em relação à semana anterior (' + Math.round(taxaAnt) + '%).';
    } else {
      fraseTaxa += '.';
    }
  } else {
    fraseTaxa = 'Sem tentativas nos últimos 7 dias para calcular taxa de sucesso.';
  }

  var meta = window.ST.intimidade.meta;
  if (meta) frase += ' Meta do casal: ' + meta + '/semana.';

  var tabelaEvolucao = '<table class="tbl" style="margin-top:10px;"><tr><th>Semana</th><th>Tentativas</th><th>Consumados</th><th>Taxa</th></tr>';
  for (var s = 5; s >= 0; s--) {
    var ini = s * 7, fim = s * 7 + 6, t = 0, c = 0;
    for (var k = 0; k < arr.length; k++) {
      var dk = diffDias(arr[k].data, hojeISO());
      if (dk >= ini && dk <= fim) { t++; if (arr[k].consumado === 'sim') c++; }
    }
    var taxaSemana = t > 0 ? Math.round(c / t * 100) + '%' : '—';
    var rotuloSemana = s === 0 ? 'Esta semana' : 'Há ' + s + ' sem.';
    tabelaEvolucao += '<tr><td>' + rotuloSemana + '</td><td>' + t + '</td><td>' + c + '</td><td>' + taxaSemana + '</td></tr>';
  }
  tabelaEvolucao += '</table>';

  return '<div class="card ai-box"><div class="card-title">💡 Insight</div>' + frase + '<br><br>' + fraseTaxa + tabelaEvolucao + '</div>';
}

function renderMetaIntimidadeCard() {
  var meta = window.ST.intimidade.meta;
  var html = '<div class="card"><div class="card-title">🎯 Meta do casal</div>';
  html += '<div class="card-sub">Não existe um número "correto" definido pela ciência — essa meta é só o que vocês dois consideram bom.</div>';
  html += '<div class="row2">';
  html += '<div class="field"><label>Frequência desejada (por semana)</label><input class="input" type="number" step="0.5" id="intimMetaInput" value="' + (meta !== null && meta !== undefined ? meta : '') + '"></div>';
  html += '<div class="field" style="display:flex;align-items:flex-end;"><button type="button" class="btn btn-ghost" style="width:100%;" onclick="salvarMetaIntimidade()">Salvar meta</button></div>';
  html += '</div></div>';
  return html;
}

function salvarMetaIntimidade() {
  var val = Number(document.getElementById('intimMetaInput').value) || null;
  window.ST.intimidade.meta = val;
  window.saveState();
  renderIntimidade();
}
window.salvarMetaIntimidade = salvarMetaIntimidade;

function renderCruzamentoLazerIntimidadeCard() {
  var html = '<div class="card"><div class="card-title">🔗 Lazer (casal) × Intimidade</div>';
  html += '<div class="card-sub">Frequência dos programas a dois e da intimidade, semana a semana, para ver se uma acompanha a outra.</div>';
  html += '<table class="tbl"><tr><th>Semana</th><th>Lazer a dois</th><th>Tentativas</th><th>Consumados</th></tr>';
  for (var s = 7; s >= 0; s--) {
    var inicioJanela = s * 7, fimJanela = s * 7 + 6;
    var lazerCount = 0, tentativas = 0, consumados = 0;
    for (var i = 0; i < window.ST.juntos.momentos.length; i++) {
      if (window.ST.juntos.momentos[i].quem !== 'casal') continue;
      var d1 = diffDias(window.ST.juntos.momentos[i].data, hojeISO());
      if (d1 >= inicioJanela && d1 <= fimJanela) lazerCount++;
    }
    for (var j = 0; j < window.ST.intimidade.encontros.length; j++) {
      var d2 = diffDias(window.ST.intimidade.encontros[j].data, hojeISO());
      if (d2 >= inicioJanela && d2 <= fimJanela) {
        tentativas++;
        if (window.ST.intimidade.encontros[j].consumado === 'sim') consumados++;
      }
    }
    var rotulo = s === 0 ? 'Esta semana' : 'Há ' + s + ' semana(s)';
    html += '<tr><td>' + rotulo + '</td><td>' + lazerCount + '</td><td>' + tentativas + '</td><td>' + consumados + '</td></tr>';
  }
  html += '</table></div>';
  return html;
}

// ===================== INSIGHT: BÍBLIA (sequência de leitura) =====================
function calcularSequenciaBiblia() {
  var datas = {};
  for (var i = 0; i < window.ST.biblia.leituras.length; i++) datas[window.ST.biblia.leituras[i].data] = true;
  var streak = 0;
  var cursor = hojeISO();
  if (!datas[cursor]) cursor = addDias(cursor, -1);
  while (datas[cursor]) { streak++; cursor = addDias(cursor, -1); }
  return streak;
}

function renderInsightBibliaCard() {
  var streak = calcularSequenciaBiblia();
  var ultimos7 = 0;
  for (var i = 0; i < window.ST.biblia.leituras.length; i++) {
    var d = diffDias(window.ST.biblia.leituras[i].data, hojeISO());
    if (d >= 0 && d <= 6) ultimos7++;
  }
  var frase = streak > 0 ? streak + ' dia(s) seguido(s) de leitura 🔥' : 'Sem sequência ativa no momento';
  frase += ' · ' + ultimos7 + ' leitura(s) nos últimos 7 dias';
  return '<div class="card ai-box"><div class="card-title">💡 Insight</div>' + frase + '</div>';
}

// ===================== GRÁFICO DE BARRAS: SONO COMPARATIVO (últimos 7 dias) =====================
...
function renderGraficoBarrasSono() {
  var hoje = hojeISO();
  var datas = [];
  for (var i = 6; i >= 0; i--) datas.push(addDias(hoje, -i));

  var valores = datas.map(function (d) {
    var regT = null, regA = null;
    for (var j = 0; j < window.ST.tarcisio.sono.length; j++) if (window.ST.tarcisio.sono[j].data === d) { regT = window.ST.tarcisio.sono[j]; break; }
    for (var k = 0; k < window.ST.aline.sono.length; k++) if (window.ST.aline.sono[k].data === d) { regA = window.ST.aline.sono[k]; break; }
    var minT = regT ? calcSonoReal(regT.dormir, regT.acordar, regT.intervalo) : null;
    var minA = regA ? calcSonoReal(regA.dormir, regA.acordar, regA.intervalo) : null;
    return { data: d, t: minT !== null ? minT / 60 : null, a: minA !== null ? minA / 60 : null };
  });

  var max = 1;
  for (var m = 0; m < valores.length; m++) {
    if (valores[m].t && valores[m].t > max) max = valores[m].t;
    if (valores[m].a && valores[m].a > max) max = valores[m].a;
  }
  max = Math.ceil(max);

  var W = 340, H = 180, PAD_BOTTOM = 24, PAD_TOP = 10;
  var slotW = (W - 20) / 7;
  var barW = slotW * 0.35;
  var svg = '<svg viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;height:auto;">';
  for (var n = 0; n < valores.length; n++) {
    var v = valores[n];
    var slotX = 10 + n * slotW;
    if (v.t !== null) {
      var hT = (v.t / max) * (H - PAD_BOTTOM - PAD_TOP);
      svg += '<rect x="' + slotX + '" y="' + (H - PAD_BOTTOM - hT) + '" width="' + barW + '" height="' + hT + '" fill="#1A6B9A" rx="3"/>';
    }
    if (v.a !== null) {
      var hA = (v.a / max) * (H - PAD_BOTTOM - PAD_TOP);
      svg += '<rect x="' + (slotX + barW + 3) + '" y="' + (H - PAD_BOTTOM - hA) + '" width="' + barW + '" height="' + hA + '" fill="#C2185B" rx="3"/>';
    }
    svg += '<text x="' + (slotX + barW) + '" y="' + (H - 6) + '" font-size="10" fill="var(--muted)" text-anchor="middle">' + diaSemanaAbrev(v.data) + '</text>';
  }
  svg += '</svg>';

  var legenda = '<div style="display:flex;gap:16px;margin-bottom:8px;font-size:12px;">';
  legenda += '<span style="display:flex;align-items:center;gap:4px;"><i style="width:10px;height:10px;background:#1A6B9A;border-radius:2px;display:inline-block;"></i>Tarcísio</span>';
  legenda += '<span style="display:flex;align-items:center;gap:4px;"><i style="width:10px;height:10px;background:#C2185B;border-radius:2px;display:inline-block;"></i>Aline</span>';
  legenda += '</div>';

  return '<div class="card"><div class="card-title">📊 Sono comparativo (últimos 7 dias)</div>' + legenda + svg + '</div>';
}

// ===================== GRÁFICO DE BARRAS: TREINO (minutos por dia, últimos 7 dias) =====================
function renderGraficoBarrasTreino() {
  var hoje = hojeISO();
  var datas = [];
  for (var i = 6; i >= 0; i--) datas.push(addDias(hoje, -i));
  ...
  var valores = datas.map(function (d) {
    var minT = 0, minA = 0;
    for (var j = 0; j < window.ST.tarcisio.treinos.length; j++) {
      if (window.ST.tarcisio.treinos[j].data === d) { var dt = calcDuracaoAtividade(window.ST.tarcisio.treinos[j].inicio, window.ST.tarcisio.treinos[j].fim); if (dt) minT += dt; }
    }
    for (var k = 0; k < window.ST.aline.treinos.length; k++) {
      if (window.ST.aline.treinos[k].data === d) { var da = calcDuracaoAtividade(window.ST.aline.treinos[k].inicio, window.ST.aline.treinos[k].fim); if (da) minA += da; }
    }
    return { data: d, t: minT || null, a: minA || null };
  });

  return renderBarrasDuasSeries(valores, '📊 Treino comparativo (últimos 7 dias)', 'min');
}

// ===================== GRÁFICO DE BARRAS: ÁLCOOL (doses por dia, últimos 7 dias) =====================
function renderGraficoBarrasAlcool() {
  var hoje = hojeISO();
  var datas = [];
  for (var i = 6; i >= 0; i--) datas.push(addDias(hoje, -i));
  ...
  var valores = datas.map(function (d) {
    var doseT = 0, doseA = 0;
    for (var j = 0; j < window.ST.tarcisio.alcool.length; j++) if (window.ST.tarcisio.alcool[j].data === d) doseT += (window.ST.tarcisio.alcool[j].quantidade || 0);
    for (var k = 0; k < window.ST.aline.alcool.length; k++) if (window.ST.aline.alcool[k].data === d) doseA += (window.ST.aline.alcool[k].quantidade || 0);
    return { data: d, t: doseT || null, a: doseA || null };
  });

  return renderBarrasDuasSeries(valores, '📊 Álcool comparativo (últimos 7 dias)', 'doses');
}

// Função genérica de barras agrupadas (2 séries) por dia da semana — reaproveitada por Treino e Álcool
function renderBarrasDuasSeries(valores, titulo, unidade) {
  var max = 1;
  for (var m = 0; m < valores.length; m++) {
    if (valores[m].t && valores[m].t > max) max = valores[m].t;
    if (valores[m].a && valores[m].a > max) max = valores[m].a;
  ...
  return '<div class="card"><div class="card-title">' + titulo + '</div>' + legenda + svg + '</div>';
}

// ===================== GRÁFICO DE BARRAS: LAZER (eventos por semana, últimas 8 semanas) =====================
function renderGraficoBarrasLazer() {
  var contagens = [];
  for (var s = 7; s >= 0; s--) {
    var ini = s * 7, fim = s * 7 + 6, total = 0;
    for (var i = 0; i < window.ST.juntos.momentos.length; i++) {
      var d = diffDias(window.ST.juntos.momentos[i].data, hojeISO());
      if (d >= ini && d <= fim) total++;
    }
    contagens.push({ rotulo: s === 0 ? 'Agora' : s + 'sem', valor: total });
  }
  var max = 1;
  for (var m = 0; m < contagens.length; m++) if (contagens[m].valor > max) max = contagens[m].valor;

  var W = 340, H = 160, PAD_BOTTOM = 22, PAD_TOP = 10;
  var slotW = (W - 20) / 8;
  var barW = slotW * 0.55;
  var svg = '<svg viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;height:auto;">';
  for (var n = 0; n < contagens.length; n++) {
    var v = contagens[n];
    var slotX = 10 + n * slotW + (slotW - barW) / 2;
    var hBar = (v.valor / max) * (H - PAD_BOTTOM - PAD_TOP);
    svg += '<rect x="' + slotX + '" y="' + (H - PAD_BOTTOM - hBar) + '" width="' + barW + '" height="' + hBar + '" fill="var(--primary)" rx="3"/>';
    if (v.valor > 0) svg += '<text x="' + (slotX + barW / 2) + '" y="' + (H - PAD_BOTTOM - hBar - 4) + '" font-size="10" fill="var(--txt)" text-anchor="middle">' + v.valor + '</text>';
    svg += '<text x="' + (slotX + barW / 2) + '" y="' + (H - 6) + '" font-size="9" fill="var(--muted)" text-anchor="middle">' + v.rotulo + '</text>';
  }
  svg += '</svg>';
  return '<div class="card"><div class="card-title">📊 Eventos de lazer por semana</div>' + svg + '</div>';
}

// ===================== GRÁFICO DE LINHA: DURAÇÃO DOS CICLOS AO LONGO DO TEMPO =====================
function renderGraficoCicloCard() {
  var ciclos = window.ST.aline.ciclos;
  var html = '<div class="card"><div class="card-title">📊 Duração dos ciclos ao longo do tempo</div>';
  if (ciclos.length < 2) {
    html += '<div class="li-sub">Registre pelo menos 2 ciclos para ver o gráfico.</div></div>';
    return html;
  }
  var ord = ciclosOrdenados(ciclos);
  var pontos = [];
  for (var i = 0; i < ord.length; i++) {
    var info = infoCiclo(ord[i], ciclos);
    pontos.push({ x: ord[i].dataInicio, y: info.duracaoReal });
  }
  html += renderGraficoLinha(pontos, '#9C7A54');
  html += '</div>';
  return html;
}

// ===================== LINHA DO TEMPO: USO CONTÍNUO (remédios/suplementos) =====================
function renderGraficoPeriodosCard() {
  var lista = window.ST.periodos.lista.slice().sort(function (a, b) { return a.dataInicio < b.dataInicio ? -1 : 1; });
  var html = '<div class="card"><div class="card-title">📊 Linha do tempo — uso contínuo</div>';
  if (lista.length === 0) {
    html += '<div class="li-sub">Nenhum uso contínuo registrado ainda.</div></div>';
    return html;
  }
  var hoje = hojeISO();
  var minD = lista[0].dataInicio;
  for (var i = 1; i < lista.length; i++) if (lista[i].dataInicio < minD) minD = lista[i].dataInicio;
  var totalDias = Math.max(diffDias(minD, hoje), 1);

  html += '<div style="display:flex;flex-direction:column;gap:12px;">';
  for (var j = 0; j < lista.length; j++) {
    var p = lista[j];
    var fimEfetivo = p.dataFim || hoje;
    var offsetPct = (diffDias(minD, p.dataInicio) / totalDias) * 100;
    var larguraPct = Math.max((diffDias(p.dataInicio, fimEfetivo) / totalDias) * 100, 1.5);
    var cor = p.quem === 'tarcisio' ? '#1A6B9A' : '#C2185B';
    var icone = p.tipo === 'remedio' ? '💊' : '🧴';
    var emUso = !p.dataFim;
    html += '<div>';
    html += '<div style="font-size:12px;font-weight:600;color:var(--txt);margin-bottom:4px;">' + icone + ' ' + esc(p.nome) + (emUso ? ' <span style="color:#2D6A4F;">(em uso)</span>' : '') + '</div>';
    html += '<div style="position:relative;background:var(--bg);border-radius:6px;height:14px;width:100%;">';
    html += '<div style="position:absolute;left:' + offsetPct + '%;width:' + larguraPct + '%;background:' + cor + ';height:100%;border-radius:6px;"></div>';
    html += '</div>';
    html += '</div>';
  }
  html += '</div>';
  html += '<div class="li-sub" style="margin-top:12px;">' + fmtDate(minD) + ' até hoje (' + fmtDate(hoje) + ')</div>';
  html += '<div class="legend" style="margin-top:6px;"><span><i style="background:#1A6B9A;"></i>Tarcísio</span><span><i style="background:#C2185B;"></i>Aline</span></div>';
  html += '</div>';
  return html;
}

// ===================== CAPTURA DE CAMPOS DO TREINO (evita perda ao trocar tipo) =====================
window._treinoCamposTemp = {};
var CAMPOS_TREINO_TEXTO = ['treinoDistancia', 'treinoFcMedia', 'treinoCalorias'];
function capturarCamposTreino() {
  for (var i = 0; i < CAMPOS_TREINO_TEXTO.length; i++) {
    var elT = document.getElementById(CAMPOS_TREINO_TEXTO[i]);
    if (elT) window._treinoCamposTemp[CAMPOS_TREINO_TEXTO[i]] = elT.value;
  }
}
window.capturarCamposTreino = capturarCamposTreino;
function valorCampoTreino(id, padrao) {
  if (window._treinoCamposTemp[id] !== undefined) return window._treinoCamposTemp[id];
  return padrao === undefined ? '' : padrao;
}

// ===================== PERFORMANCE DE CORRIDA (ritmo/pace ao longo do tempo) =====================
window._perfCorridaPessoa = window._perfCorridaPessoa || 'tarcisio';

function calcRitmo(distanciaKm, duracaoMin) {
  if (!distanciaKm || !duracaoMin) return null;
  return duracaoMin / distanciaKm; // min/km
}
function formatarRitmo(ritmo) {
  if (ritmo === null || ritmo === undefined) return '—';
  var minutos = Math.floor(ritmo);
  var segundos = Math.round((ritmo - minutos) * 60);
  if (segundos === 60) { minutos++; segundos = 0; }
  return minutos + "'" + pad2(segundos) + '"/km';
}

function setPerfCorridaPessoa(quem) {
  window._perfCorridaPessoa = quem;
  renderTreino();
}
window.setPerfCorridaPessoa = setPerfCorridaPessoa;

function renderPerformanceCorridaCard() {
  var pessoa = window._perfCorridaPessoa;
  var nomeLabel = pessoa === 'tarcisio' ? 'Tarcísio' : 'Aline';
  var corridas = window.ST[pessoa].treinos.filter(function (t) { return t.tipo === 'corrida' && t.distancia; }).slice().sort(function (a, b) { return a.data < b.data ? -1 : 1; });

  var html = '<div class="card"><div class="card-title">🏃‍♂️ Performance de corrida</div>';
  html += '<div class="period-pick" style="margin-bottom:10px;">';
  html += '<button type="button" class="' + (pessoa === 'tarcisio' ? 'on' : '') + '" onclick="setPerfCorridaPessoa(\'tarcisio\')">👨 Tarcísio</button>';
  html += '<button type="button" class="' + (pessoa === 'aline' ? 'on' : '') + '" onclick="setPerfCorridaPessoa(\'aline\')">👩 Aline</button>';
  html += '</div>';

  if (corridas.length === 0) {
    html += '<div class="li-sub">Nenhuma corrida com distância registrada ainda para ' + nomeLabel + '.</div></div>';
    return html;
  }

  var distanciaTotal = 0, melhorRitmo = null;
  var pontos = [];
  for (var i = 0; i < corridas.length; i++) {
    var c = corridas[i];
    distanciaTotal += c.distancia;
    var duracao = calcDuracaoAtividade(c.inicio, c.fim);
    var ritmo = calcRitmo(c.distancia, duracao);
    if (ritmo !== null) {
      pontos.push({ x: c.data, y: ritmo });
      if (melhorRitmo === null || ritmo < melhorRitmo) melhorRitmo = ritmo;
    }
  }

  html += '<div class="stat-grid">';
  html += '<div class="stat-box"><div class="num">' + fmt1(distanciaTotal) + 'km</div><div class="lbl">Distância total</div></div>';
  html += '<div class="stat-box"><div class="num">' + formatarRitmo(melhorRitmo) + '</div><div class="lbl">Melhor ritmo (recorde)</div></div>';
  html += '</div>';

  html += '<div class="card-sub" style="font-weight:700;color:var(--txt);margin-top:12px;">Ritmo ao longo do tempo (min/km)</div>';
  var cor = pessoa === 'tarcisio' ? '#1A6B9A' : '#C2185B';
  html += renderGraficoLinha(pontos, cor, formatarRitmo);
  html += '</div>';
  return html;
}
